  ...
  class Order
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Printful::Order` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Printful::Order`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'branding_items')
        if (value = attributes[:'branding_items']).is_a?(Array)
          self.branding_items = value
        end
      end

      if attributes.key?(:'incomplete_items')
        if (value = attributes[:'incomplete_items']).is_a?(Array)
          self.incomplete_items = value
        end
      end
      ...
      if attributes.key?(:'pricing_breakdown')
        if (value = attributes[:'pricing_breakdown']).is_a?(Array)
          self.pricing_breakdown = value
        end
      end
