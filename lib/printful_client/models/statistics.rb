...
module Printful
  ...
  class Statistics
    ...
    attr_accessor :store_statistics
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Printful::Statistics` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Printful::Statistics`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'store_statistics')
        if (value = attributes[:'store_statistics']).is_a?(Array)
    ...
    def ==(o)
      ...
      self.class == o.class &&
          store_statistics == o.store_statistics
    end
