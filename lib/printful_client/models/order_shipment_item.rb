  ...
  class OrderShipmentItem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Printful::OrderShipmentItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Printful::OrderShipmentItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'picked')
        self.picked = attributes[:'picked']
      end

      if attributes.key?(:'printed')
    ...
    def valid?
      ...
      picked_validator = EnumAttributeValidator.new('Integer', [0, 1])
      return false unless picked_validator.valid?(@picked)
      printed_validator = EnumAttributeValidator.new('Integer', [0, 1])
      return false unless printed_validator.valid?(@printed)
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] picked Object to be assigned
    def picked=(picked)
      validator = EnumAttributeValidator.new('Integer', [0, 1])
      unless validator.valid?(picked)
        fail ArgumentError, "invalid value for \"picked\", must be one of #{validator.allowable_values}."
      end
      @picked = picked
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] printed Object to be assigned
    def printed=(printed)
      validator = EnumAttributeValidator.new('Integer', [0, 1])
      unless validator.valid?(printed)
        fail ArgumentError, "invalid value for \"printed\", must be one of #{validator.allowable_values}."
      end
      @printed = printed
    end
    ...
    def hash
      [item_id, quantity, picked, printed].hash
    end
