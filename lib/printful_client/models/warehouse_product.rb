...
module Printful
  ...
  class WarehouseProduct
    ...
    # Product status:   **created** - product request created,   **active** - product request approved   **suspended** - product suspended   **declined** - product request declined   **draft** - product created as a draft
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Printful::WarehouseProduct` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Printful::WarehouseProduct`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('String', ["created", "active", "suspended", "declined", "draft"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["created", "active", "suspended", "declined", "draft"])
    ...
    def hash
      [id, name, status, currency, image_url, retail_price, variants].hash
    end
