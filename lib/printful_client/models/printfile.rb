  ...
  class Printfile
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Printful::Printfile` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Printful::Printfile`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'fill_mode')
        self.fill_mode = attributes[:'fill_mode']
      end

      if attributes.key?(:'can_rotate')
        self.can_rotate = attributes[:'can_rotate']
      end
    end
    ...
    def valid?
      ...
      fill_mode_validator = EnumAttributeValidator.new('String', ["cover", "fit"])
    ...
    def fill_mode=(fill_mode)
      validator = EnumAttributeValidator.new('String', ["cover", "fit"])
    ...
    def hash
      [printfile_id, width, height, dpi, fill_mode, can_rotate].hash
    end
