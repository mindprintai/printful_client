  ...
  class SyncProduct
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Printful::SyncProduct` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Printful::SyncProduct`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@thumbnail.nil? && @thumbnail.to_s.length > 250
        invalid_properties.push('invalid value for "thumbnail", the character length must be smaller than or equal to 250.')
      end
    ...
    def valid?
      ...
      return false if !@thumbnail.nil? && @thumbnail.to_s.length > 250
      true
    end
    ...
    def thumbnail=(thumbnail)
      if thumbnail.nil?
        fail ArgumentError, 'thumbnail cannot be nil'
      end

      if thumbnail.to_s.length > 250
        fail ArgumentError, 'invalid value for "thumbnail", the character length must be smaller than or equal to 250.'
      end
    ...
    def hash
      [id, external_id, name, variants, synced, thumbnail, thumbnail_url, is_ignored].hash
    end
