...
module Printful
  class GetTaxCountries200ResponseResultInnerAllOfStatesInner
    # Whether shipping is taxable in this state
    attr_accessor :shipping_taxable
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Printful::GetTaxCountries200ResponseResultInnerAllOfStatesInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Printful::GetTaxCountries200ResponseResultInnerAllOfStatesInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'shipping_taxable')
        self.shipping_taxable = attributes[:'shipping_taxable']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          shipping_taxable == o.shipping_taxable
    end
